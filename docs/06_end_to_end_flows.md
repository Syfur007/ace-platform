...
- DB table names to inspect for these flows: `users`, `auth_sessions`, `auth_refresh_tokens`, `practice_sessions`, `practice_answers` (see `apps/backend/internal/db/db.go` and `apps/backend/internal/handlers/practice.go`).
